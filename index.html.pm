  ...
  (header ([class "lg:sticky lg:top-0 lg:flex lg:max-h-screen lg:w-1/2 lg:flex-col lg:justify-between lg:py-20"])
	...
	(section ([id "about"] [class "mb-16 group/section scroll-mt-16 lg:mb-0 lg:scroll-mt-0"] [aria-label "About me"])
			 ...
			 (div ([class "lg:w-10/12"])
			 	  ...
			 	  (p ([class "mt-4"])
				     "Currently learning how to make my life simpler with " ,(link "alles" "https://github.com/haglobah/alles") " & " ,(link "Nix" "https://nixos.org") " and practicing to type faster.")
			 	  (p ([class "mt-4"])
	...
	(ul ([class "ml-1 mt-8 flex items-center"])
	  ...
	  (li ([class "mr-5 text-sm"])
	      (a ([class "block hover:text-zinc-200"] [href "https://www.linkedin.com/in/beat-hagenlocher/"] [target "_blank"] [rel "noreferrer"])
		     (span ([class "sr-only"]))
	  ...
	  (li ([class "mr-5 text-sm"])
	      (a ([class "block hover:text-zinc-200"] [href "mailto:bahREMOVETHIS@posteo.de"] [target "_blank"] [rel "noreferrer"])
		     (span ([class "sr-only"]))
