...
(define (relevant-link name href)
    `(li ([class "mr-4"])
            (a ([class "relative inline-flex items-center text-sm font-medium text-slate-300 hover:text-[#9BBEFF] focus-visible:text-[#9BBEFF]"] [href ,href] [target "_blank"] [rel "noreferrer"])
...
(define (section title aria-label . content)
    `(section ([id ,(string-downcase title)] [class "group/section mb-16 scroll-mt-16 md:mb-24 lg:mb-36 lg:scroll-mt-24"] [aria-label ,aria-label])
